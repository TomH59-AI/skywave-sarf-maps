...
// Master login credentials (set in Railway env vars)
...
app.post('/api/master-login', (req, res) => {
    ...
    if (!MASTER_EMAIL || !MASTER_PASSWORD) {
          return res.status(500).json({ error: 'Master login not configured' });
    }
    if (email === MASTER_EMAIL && password === MASTER_PASSWORD) {
          return res.json({ success: true, token: 'master_' + Date.now(), role: 'admin' });
    }
...
app.post('/stripe/create-checkout-session', async (req, res) => {
    try {
      ...
      if (priceType === 'scip_one') {
              priceId = PRICE_SCIP;
              mode = 'payment';
      } else if (priceType === 'pro_monthly') {
              priceId = PRICE_PRO;
              mode = 'subscription';
      } else if (priceType === 'pro_annual') {
      ...
      const params = {
              ...
              mode,
              success_url: successUrl || (req.headers.origin + '/app?paid=1&plan=' + priceType),
